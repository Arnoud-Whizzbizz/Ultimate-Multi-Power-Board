...
//
//   SDL_Arduino_INA3221::setCriticalAlertValue was added by Arnoud van Delden (Whizzbizz.com) - June 2022
//
...
void SDL_Arduino_INA3221::INA3221SetConfig(void) {
  ...
  uint16_t config = INA3221_CONFIG_RESET |
...
/**************************************************************************/
/*! 
    AvD: calculate binary value from critial current in milli amps
*/
/**************************************************************************/
void SDL_Arduino_INA3221::setCriticalAlertValue(float mAmps, int channel) {    
  uint16_t shuntVolts;
  uint16_t binValue;

  shuntVolts = (mAmps * 2.5); // Value: ( A * Rshunt ) / 40uV
  binValue = shuntVolts << 3; // Three lowest bits are 0

  //Serial.print("\nmAmps=");
  //Serial.println(mAmps);
  //Serial.print("\nshuntVolts=");
  //Serial.println(shuntVolts);
  //Serial.print("binValue=");
  //Serial.println(binValue, BIN);

  wireWriteRegister(INA3221_CRITICAL_1+(channel-1)*2, binValue);
}
...
int16_t SDL_Arduino_INA3221::getBusVoltage_raw(int channel) {
  ...
  //Serial.println(INA3221_REG_BUSVOLTAGE_1+(channel -1) *2);
...
int16_t SDL_Arduino_INA3221::getShuntVoltage_raw(int channel) {
  ...
  //Serial.println(channel);
  //Serial.print("Pointeraddr=");
  //Serial.println(INA3221_REG_SHUNTVOLTAGE_1+(channel -1) *2);
